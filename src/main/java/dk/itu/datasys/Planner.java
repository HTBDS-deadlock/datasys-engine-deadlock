...
public final class Planner {
    ...
    public Plan plan(SelectStatement statement) {
        ...
        if (filters.isEmpty()) {
            ...
            Operator root = new ScanOperator(engine, tableName, allPartitions);
            return new Plan(root, new ScanStats(partitionsTotal, partitionsTotal, 0));
        }
        ...
        ScanStats stats = new ScanStats(partitionsTotal, partitionsRead, partitionsTotal - partitionsRead);

        Operator scan = new ScanOperator(engine, tableName, survivingPartitions);
        Operator root = new FilterOperator(scan, engine.schema(tableName), predicate);
